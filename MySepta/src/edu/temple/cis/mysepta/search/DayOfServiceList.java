...
public class DayOfServiceList extends ListActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			septaDB.open();
			dayOfServiceList = septaDB.getDayOfServiceList(route);
			septaDB.close();
		} catch (Exception e) {
	...
	public class RouteListAdapter extends BaseAdapter {
		...
		public Object getItem(int position) {
			return dayOfServiceList.get(position);
		}
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			DayOfService dayOfservice = dayOfServiceList.get(position);
			...
			text.setText(dayOfservice.toString());
			
			layout.setTag(dayOfservice);
			layout.addView(text, params);
			convertView = layout;
			convertView.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View view) {
					DayOfService dayOfservice = (DayOfService) dayOfServiceList.get(position);
					Intent intent = new Intent(view.getContext(), MFL.class);
					Bundle bundle = new Bundle();
					bundle.putSerializable("dayOfservice", dayOfservice);
