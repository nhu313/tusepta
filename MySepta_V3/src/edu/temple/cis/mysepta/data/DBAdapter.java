	...
	public String nhuTag = "__________________";
	   ...
	   //For Schedule Table ---------------------------------------------
	    ...
	    private static final String DATABASE_CREATE_Schedule =
	    	...
	    	+ KEY_StopNameID + " integer not null, " + KEY_Schedule + " REAL not null);";
	    ...
	    protected Cursor getFavSchedule(){
	    	float t = MySeptaDataHelper.getTime();
	    	String sql = "Select Route.routeShortName, Stop.stop, Schedule.schedule "
	    		+ "from Route INNER JOIN DayOfService On Route.routeID = DayOfService.RouteID "
	    		+ "INNER JOIN Stop ON DayOfService.dayOfServiceID = Stop.dayOfServiceID "
	    		+ "INNER JOIN Schedule ON Stop.stopID = Schedule.stopID "
	    		+ "WHERE Stop.favoriteRoute = 1 AND schedule >= " + t + " order by Schedule.schedule limit 30;";
	    	return db.rawQuery(sql, null);
	    }
	    
	    protected Cursor getFavScheduleByStopId(long stopId){
	    	float t = MySeptaDataHelper.getTime();
	    	String sql = "Select Route.routeShortName, Stop.stop, Schedule.schedule "
	    		+ "from Route INNER JOIN DayOfService On Route.routeID = DayOfService.RouteID "
	    		+ "INNER JOIN Stop ON DayOfService.dayOfServiceID = Stop.dayOfServiceID "
	    		...
	    		+ " order by Schedule.schedule limit 30;";
	    	return db.rawQuery(sql, null);
	    }
}