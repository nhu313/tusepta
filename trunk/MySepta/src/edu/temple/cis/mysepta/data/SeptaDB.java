...
public class SeptaDB extends DBAdapter{
	...
	/**
	 * Retrieve the favorite schedule for the "Home" screen.
	 * @param stopId Stop ID of the schedule to get. 0 indicate all.
	 * @param time Time to search for.
	 * @return MySchedule listing consisting of route, stop name, and schedule.
	 */
	public List<MySchedule> getMySchedule(long stopId, float time){
		List<MySchedule> list = new ArrayList<MySchedule>();
		Cursor c = null;
		if (stopId == 0){
			c = super.getFavSchedule(time);
		} else {
			c = super.getFavScheduleByStopId(stopId, time);
		}
		if (c.moveToFirst()){
			for (int i = 0; i < c.getCount(); i++){
	...
	/**
	 * Retrieve favorite stop list.
	 * @return List of favorite stop consisting of the route name and stop.
	 */
